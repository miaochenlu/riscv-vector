  ...
  def vLsuTest1(): Unit =
    it should "pass: unit-stride load (uops=2, eew=8, vl=19, vstart=0)" in {
      test(new SmartVectorTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        ...
        val ldReqs = Seq(
          (
            ldstReqCtrl_default.copy(instrn = VLE8_V, vl = 19, vlmul = 2, ma = true, ta = true),
            ldstReqSrc_default.copy(),
          )
        )
  ...
  def vLsuTest5(): Unit =
    it should "pass: unit-stride load (uops=2, eew=64, vl=3, vstart=1)" in {
      test(new SmartVectorTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        ...
        val ldReqs = Seq(
          (
            ldstReqCtrl_default.copy(instrn = VLE64_V, vl = 3, vlmul = 1, vstart = 1, vsew = 3),
            ldstReqSrc_default.copy(),
          )
        )
  ...
  def vLsuTest6(): Unit =
    it should "pass: strided load (uops=1, eew=8, vl=6, vstart=0, stride=-5)" in {
      test(new SmartVectorTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        ...
        val ldReqs = Seq(
          (
            ldstReqCtrl_default.copy(instrn = VLSE8_V, vl = 6, vlmul = 1, vsew = 0),
            ldstReqSrc_default.copy(rs2 = "hffffffff_fffffffb"),
          )
        )
  ...
  def vLsuTest7(): Unit =
    it should "pass: strided load (uops=2, eew=64, vl=3, vstart=0, stride=-1)" in {
      test(new SmartVectorTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        ...
        val ldReqs = Seq(
          (
            ldstReqCtrl_default.copy(instrn = VLSE32_V, vl = 3, vlmul = 1, vsew = 2),
            ldstReqSrc_default.copy(rs2 = "hffffffff_ffffffff"),
          )
        )
  ...
  def vLsuTest8(): Unit =
    it should "pass: strided load (uops=2, eew=16, vl=10, vstart=0, stride=4)" in {
      test(new SmartVectorTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        ...
        val ldReqs = Seq(
          (
            ldstReqCtrl_default.copy(instrn = VLSE16_V, vl = 10, vlmul = 1, vsew = 1),
            ldstReqSrc_default.copy(rs2 = "h8"),
          )
        )
  ...
  def vLsuTest9(): Unit =
    it should "pass: unit-stride exception" in {
      test(new SmartVectorTestWrapper).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        ...
        val ldReqs = Seq(
          (
            ldstReqCtrl_default.copy(instrn = VSE8_V, vl = 0, vlmul = 1, vstart = 1, vsew = 0),
            ldstReqSrc_default.copy(rs1 = "h1058"),
          ),
          (
            ldstReqCtrl_default.copy(instrn = VLE8_V, vl = 19, vlmul = 1, vstart = 1, vsew = 0),
            ldstReqSrc_default.copy(rs1 = "h1058"),
          ),
        )
